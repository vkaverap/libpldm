				     ...
				     uint32_t bmc_record_handle, bool hotplug)
{
	int rc = pldm_pdr_add_fru_record_set_check(
		...
		entity_instance_num, container_id, &bmc_record_handle, hotplug);
	(void)rc;
	assert(!rc);
				      ...
				      bool hotplug)
{
	...
	if (hotplug) {
		...
		*bmc_record_handle = pldm_pdr_add(repo, data, size, *bmc_record_handle, false,
				    fru->terminus_handle);
		return 0;
	}
}
	...
	uint32_t record_handle)
{
	int rc = pldm_entity_association_pdr_add_from_node_with_record_handle(
		node, repo, entities, num_entities, is_remote, terminus_handle, record_handle);
	(void)rc;
	assert(!rc);
}

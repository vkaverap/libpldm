...
TEST(PDRAccess, getPLDMEntityfromPDR)
{
    ...
    uint32_t handle = 0;
    int rc = pldm_pdr_add_fru_record_set_check(repo, 1, 10, 1, 0, 100, &handle, false);
    ASSERT_EQ(rc, 0);
    ...
    std::array<uint8_t, 27> data2{10, 1, 0, 0,   1, 4,   0, 0, 17,
                                  ...
                                  5,  0, 0, 0,   1, 10,  0, 1, 6};
    handle = 0;
    rc = pldm_pdr_add_check(repo, data2.data(), data2.size(), false, 1, &handle);
    ASSERT_EQ(rc, 0);
    entity = pldm_get_entity_from_record_handle(repo, handle);
    ...
    handle = 0;
    rc = pldm_pdr_add_check(repo, data3.data(), data3.size(), false, 1, &handle);
    ASSERT_EQ(rc, 0);
    entity = pldm_get_entity_from_record_handle(repo, handle);
    ...
    for (const auto& type : notSupportedtypes)
    {
        ...
        hdr->type = type;
        handle = 0;
        rc = pldm_pdr_add_check(repo, data4.data(), data4.size(), false, 1, &handle);
        ASSERT_EQ(rc, 0);
        entity = pldm_get_entity_from_record_handle(repo, handle);
...
TEST(EntityAssociationPDR, testEntityInstanceNumber)
{
    ...
    auto l1 = pldm_entity_association_tree_add(tree, &entities[1], 63, node,
                                               PLDM_ENTITY_ASSOCIAION_PHYSICAL, false, false, 0);
    uint32_t first = 1;
